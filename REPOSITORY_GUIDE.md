...
| `src/ghostShipEffect.ts` | Dead-player "spirit ship" visual: one continuous fractal organism, not per-node spirals. | `GhostShipEffect` pooled ring buffer (struct-of-arrays, hard cap). Two independent scales: a persistent growth spine whose heading is bent by a slowly drifting, seeded curvature (`heading += curvature * distanceTravelled`, gently leashed back to the real spectator anchor) so the macro path glides between straight/arc/broad-spiral/tight-spiral states; and, at each spine node, a small recursive Sierpinski/Julia-tendril-style cluster of shrinking, rotated triangles budded from a root triangle. Fragments never move after birth and age white -> faction -> black -> transparent; source-over triangle pass plus a GlowLayer halo pass at the head; quality/pool-pressure-driven density scaling (not movement-speed-driven, so normal/Shift ghost speeds keep full recursive depth). Visual only. |
| `src/ghostShipEffect.test.ts` | Vitest coverage for the spirit-ship effect. | Seed determinism, seed variation, fragment immutability, 1–4 s cooling range, tail alpha reaching zero, cap enforcement, density scaling, clear/reset behaviour, recursion depth held at normal/Shift speed, dt-stable emission density, and a genuine large-scale curved spine (vs. independent per-node spiral stamps). |
